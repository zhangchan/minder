...
_p[2] = {
    value: function(require, exports, module) {
        ...
        connect.register("line", function(link, node, connection,strokeWidth,color) {
            // 连线起点和终点
            var no = node.getLayoutVertexOut(), 
                ni = node.getLayoutVertexIn();
                to = link.getLayoutVertexOut();
                ti = link.getLayoutVertexIn();

            var t = minder.getRenderContainer().getTransform().m;
            var pathData = ["M"];
            if(no.x < to.x){
                pathData.push(no.x+t.e, no.y+t.f, ti.x+t.e, ti.y+t.f)
            }else{
                pathData.push(ni.x+t.e, ni.y+t.f, to.x+t.e, to.y+t.f)
            }
            connection.setPathData(pathData).stroke("#ff0000",1).setAttr("stroke-dasharray","3,3");
        });
    }
};
...
_p[11] = {
    value: function(require, exports, module) {
         ...
         */
            ...
            getConnectProvider: function(type) {
                return _connectProviders[type || this.getConnect()] || _connectProviders["default"];
            },
            /**
         * @private
         ...
         */
            ...
            importJson : function(data){
                ...
                this.clearChildren();
                // minder.layout(100);
                function appendChild(obj,data){
                    var node;
                    for(var i=0;i<data.length;i++){
                        node =  minder.createNode(data[i].data, obj);
                        appendChild(node,data[i].children);
                        if (obj.isExpanded()) {
                            node.render();
                        } else {
                            obj.expand();
         ...
         */
            ...
            drawRelationLine: function(){
                ...
                arr.forEach(function(ele,i){
                    var link = new kity.Path();
                    minder._paper.addShape(lineG);
                    lineG.addShape(link)
                    that.getConnectProvider("line")(ele,that,link)
                }); 
            }
        });
...
_p[53] = {
    value: function(require, exports, module) {
        ...
        var Module = _p.r(20);
        var Renderer = _p.r(27);
        Module.register("AnalysisModule", function() {
            var Analysis_PATH = "M513.023306 95.685269c-216.360699 0-391.756405 175.395706-391.756405 391.755382s175.394682 391.756405 391.756405 391.756405 391.756405-175.395706 391.756405-391.756405S729.384006 95.685269 513.023306 95.685269zM441.596534 736.314859 441.596534 238.567467 690.469718 487.441675 441.596534 736.314859z";
            /**
         * @command Analysis
         ...
         *   -1: 当前没有选中的节点
         */
            // var AnalysisCommand = kity.createClass("AnalysisCommand", {
            //     base: Command,
            //     execute: function(minder, analysis) {
            //         var node = minder.getSelectedNode();
            //         node.setData("analysis", analysis);
            //         node.render();
            ...
            //     queryValue: function(minder) {
            //         var node = minder.getSelectedNode();
            //         return node && node.getData("analysis");
            //     }
            // });
            var AnalysisIcon = kity.createClass("AnalysisIcon", {
                base: kity.Group,
                constructor: function() {
                    this.callBase();
                    this.width = 20;
                    this.height = 20;
                    this.rect = new kity.Rect(20, 20, .5, -8.5, 2).fill("transparent");
                    this.path = new kity.Path().setPathData(Analysis_PATH).setTranslate(2.5, -6.5).setStyle("transform", "scale(0.022, 0.022)");
                    this.addShapes([ this.rect, this.path ]);
                    this.on("mouseover", function() {
                        this.path.fill("red");
                    }).on("mouseout", function() {
                        this.path.fill("#272636");
                    });
                    this.setStyle("cursor", "pointer");
                }
            });
            var AnalysisIconRenderer = kity.createClass("AnalysisIconRenderer", {
                base: Renderer,
                create: function(node) {
                    var icon = new AnalysisIcon();
...
_p[61] = {
    value: function(require, exports, module) {
        ...
        var ViewDragger = kity.createClass("ViewDragger", {
            ...
            linesView : function(){
                var node = this._minder.getSelectedNode();   
                var lin = document.getElementById('lineG');            
                if(node && lin){
                    node.drawRelationLine();
                }
