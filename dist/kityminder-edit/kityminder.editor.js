    ...
    value: function(require, exports, module) {
        ...
        function InputRuntime() {
            ...
            function setupFsm() {
                ...
                minder.on("mousedown", function(e) {
                    if(minder.getSelectedNode()){
                        relationLine(minder.getSelectedNode());
                         minder.on("mousemove", function(e) {
                            if(minder.getSelectedNode()){
                                relationLine(minder.getSelectedNode());
                            }
...
angular.module('kityminderEditor').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui/directive/kityminderEditor/kityminderEditor.html',
    "<div class=\"minder-editor-container\"><div class=\"top-tab\" top-tab=\"minder\" editor=\"editor\" ng-if=\"minder\"></div><div search-box minder=\"minder\" ng-if=\"minder\"></div><div class=\"minder-editor\"></div><div class=\"navigator\" navigator minder=\"minder\" ng-if=\"minder\"></div><div id=\"modal\" modaltable minder=\"minder\" ng-if=\"minder\"></div></div>"
  );
  ...
  $templateCache.put('ui/directive/modaltable/modaltable.html',
    "<button class=\"modal_btn\" ng-click=\"hide()\">></button>"+
		...
		return {
			'zh-cn': {
				...
				'ui': {
					...
					'url_share': '脑图 URL 地址：',
					'sns_share': '社交网络分享：',
					'sns_share_text': '“{0}” - 我用脑图制作的思维导图，快看看吧！（地址：{1}）',
					...
					'redirect': '您正在打开连接 {0}，脑图不能保证连接的安全性，是否要继续？',
					'navigator': '导航器',
...
angular.module('kityminderEditor')
	.directive('kityminderEditor', ['config', 'minder.service',  function(config, minderService, revokeDialog) {
		return {
			restrict: 'EA',
...
angular.module('kityminderEditor')
    .directive('modaltable', function() {
        return {
            restrict: 'A',
            templateUrl: 'ui/directive/modaltable/modaltable.html',
            scope: {
                minder: '='
            },
            link: function($scope) {
                $scope.hide = function(){
                    $('#modal').animate({left : "102%"}, 800,function(){
...
angular.module('kityminderEditor')
    ...
    angular.module('kityminderEditor').directive('clickAnywhereButHere', ['$document', function ($document) {
        return {
...
angular.module('kityminderEditor')
    .directive('analysisBtn', function() {
        return {
            restrict: 'E',
            templateUrl: 'ui/directive/analysisBtn/analysisBtn.html',
            scope: {
                minder: '='
            },
            replace: true,
...
angular.module('kityminderEditor')
    .directive('exportimgBtn', function() {
        return {
            restrict: 'E',
            templateUrl: 'ui/directive/exportimgBtn/exportimgBtn.html',
            scope: {
                minder: '='
            },
            replace: true,
            link: function ($scope) { 
                $scope.exportImg = exportImg;
                function exportImg() {
                    var name = $('.editor-title').text();
                    minder.exportData("png").then(function(uri) {
                        download(name, uri) 
                    })
                }
...
angular.module('kityminderEditor')
    .directive('exportdataBtn', function() {
        return {
            restrict: 'E',
            templateUrl: 'ui/directive/exportdataBtn/exportdataBtn.html',
            scope: {
                minder: '='
            },
            replace: true,
            link: function ($scope) {
                ...
                function exportData() {
                    // $(this).blur();
                    // var source = minder.exportData("png");
                    // var ful = source.fulfillValue;
